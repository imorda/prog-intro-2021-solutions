        ...
        try (Reader in = new InputStreamReader(System.in)) {
            ...
            try {
                ...
                for(int i = 0; i < n; i++){
                    ...
                    for(int j = 0; j < n; j++){
                        inputMatrix[i][j] = Integer.parseInt(Character.toString(line.charAt(j)));
                    }
                }

                for(int from = 0; from < n-1; from++){
                    int[] currentLine = Arrays.copyOf(inputMatrix[from], inputMatrix[from].length);

                    for(int to = from + 1; to < n; to++){
                        assert 0 <= currentLine[to] || currentLine[to] <= 1;

                        if(currentLine[to] == 1){
                            outputMatrix[from][to] = 1;

                            for(int i = to; i < n; i++){
                                currentLine[i] -= inputMatrix[to][i];
                                currentLine[i] = modula(currentLine[i], 10);
                            }
                        }
                    }
                }
